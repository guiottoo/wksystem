    ...
    <section id="investimento" className="py-28 bg-[#0f0f0f]">
      <div className="max-w-[1280px] mx-auto px-8">
        <div className="mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mt-3 mb-4">Desenvolvimento e Manutenção</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-14">
          {devPhases.map((p, i) => (
            ...
            >
              ...
              <p className="text-gray-400 text-sm mb-5 leading-relaxed">{p.description}</p>
              <ul className="space-y-2.5">
                {p.items.map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm text-gray-400 leading-relaxed">
                    <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-[#D96C45]/60 mt-2" />
                    {item}
                  </li>
                ))}
              </ul>
        ...
        {/* Contact CTA */}
        <div className="bg-gradient-to-br from-[#D96C45]/8 to-transparent border border-[#D96C45]/20 rounded-lg p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
          ...
          <a
            ...
            className="flex-shrink-0 bg-[#D96C45] hover:bg-[#c45d38] text-white px-7 py-3.5 rounded-lg text-sm font-semibold transition-all duration-200 hover:scale-[1.02] shadow-lg shadow-[#D96C45]/20 whitespace-nowrap"
          >
            Solicitar Proposta
          </a>
        </div>
