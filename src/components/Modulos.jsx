      ...
      <div className="max-w-[1280px] mx-auto px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {modules.map((mod) => (
            <div
              key={mod.number}
              className="group bg-white/[0.03] border border-white/[0.07] rounded-lg p-6 hover:border-[#D96C45]/30 hover:bg-white/[0.05] transition-all duration-300"
            >
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-lg bg-[#D96C45]/10 border border-[#D96C45]/20 flex items-center justify-center text-[#D96C45] group-hover:bg-[#D96C45]/15 transition-colors flex-shrink-0">
                  {mod.icon}
                </div>
                <span className="text-[#D96C45]/50 text-xs font-mono font-semibold">{mod.number}</span>
              </div>
              <h3 className="text-white font-semibold text-base mb-2 group-hover:text-[#D96C45] transition-colors leading-snug">
                {mod.title}
              </h3>
